...
- `data/databases/Filings.db` retains immutable, compressed type-1 EDINET ZIPs in `filings.archive_content`; extracted members are transient and materialized from the ZIP on demand. A future external archive/cache layer may move those compressed packages out of SQLite without changing the catalog contract.
- `data/databases/Filings.db` owns the rebuildable filing catalog and compact indexes: artifact manifests, parse runs, numeric/analytical XBRL facts/contexts/units, filing taxonomies, metric lineage, data-quality issues, and data watermarks. Narrative sections are reconstructed from retained ZIPs on demand; an optional external search index may be built separately. This replaces the previously proposed `Provenance.db`; the two responsibilities are too closely related to justify separate databases.
